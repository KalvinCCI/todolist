...
<div class="todoitem <?= $todoitem['etat_valide']?'valide':'a_faire' ?>">
    ...
    <footer>
        <p><time datetime="<?= $todoitem['date_creation'] ?>">Créée_self à <?= date_format(new DateTime($todoitem['date_creation']), 'G\hi\, \l\e d/m/o')?></time></p>
        <button onclick="todoUpdate(<?= $todoitem['id'] ?>, '<?= addslashes($todoitem['intitule']) ?>')">Modifier</button>
        <div>
